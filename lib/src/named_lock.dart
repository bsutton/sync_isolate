...
class NamedLock {
        ...
        await _withHardLock(
          fn: () async {
            ...
            if (lockFiles == 0) {
              ...
              lockFiles = await _clearStaleLocks(locks, lockFiles);
              if (lockFiles == 0) {
                taken = true;
              }
            }
  ...
  Future<bool> get _validLockFileExists async {
    ...
    for (final lock in locks) {
      ...
      if (await _isOwnerLive(lockFileParts.pid)) {
        return true;
      }
    }
    return false;
  ...
  Future<int> _clearStaleLocks(List<String> locks, int lockFiles) async {
  ...
  Future<bool> _isOwnerLive(int lockOwnerPid) async =>
